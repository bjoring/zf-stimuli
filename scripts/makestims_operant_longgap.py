...
def make_stims(stimset,seed, dBval):
    ...
    for i in range(len(songname)):    
        ...
        #print(ssyll.start2)
        mid1 = (ssyll.start1.values[0] + ssyll.end1.values[0])/2
        mid2 = (ssyll.start2.values[0] + ssyll.end2.values[0])/2
        gaplength = 0.1
        blocks[:,0] = np.asarray((mid1,mid2))
        blocks[:,1] = np.asarray((mid1+gaplength,mid2+gaplength))
        blocks = blocks*Fs
        blocks = blocks.astype(int)
        gsize = int(gaplength*Fs)
        wnamp = 25000
        wndB = dBval
        if blocks[0,1] > len(song[i]):
            #middle = np.mean(blocks,1)
            #blocks[0,0] = middle[0]-int(gsize/2)-50
            blocks[0,1] = len(song[i])
        if blocks[1,1] > len(song[i]):
            #middle = np.mean(blocks,1)
            #blocks[1,0] = middle[1]-int(gsize/2)-50
            blocks[1,1] = len(song[i])
        N = 120
        ix = np.arange(N*3)
